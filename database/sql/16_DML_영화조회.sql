...
WHERE
    se_sc_num = 1 AND th_name = 'CGV강남';
    
#CGV강남에 등록된 좌석을 조회하는 쿼리
SELECT 
    se_name AS 좌석명, sc_name AS 관
FROM
    seat
        JOIN
    screen ON se_sc_num = sc_num
...
# 영화 웡카를 상영 하고 있는 극장을 조회 하는 쿼리
select * from theater jo;
select * from movie;
...
# CGV강남 1상영관에 웡카 상영 시간을 조회 하는 쿼리
SELECT 
...
WHERE
    ...
    and sh_date = '2024-02-09' and sh_time = '10:30';
    
# 2월 9일 10:30분 CGV강남 1상영관에서 상영하는 웡카 예매 가능한 좌석을 조회하는 쿼리
SELECT 
    sh_date, sh_time, se_name
FROM
    seat
        JOIN
    screen ON se_sc_num = sc_th_num
        JOIN
    theater ON sc_th_num = th_num
        JOIN
...
# CGV강남 1상영관 2월 9일 10:30 웡카를 예매한 좌석을 조회
SELECT 
    *
FROM
    ticketing_list
        JOIN
    schedule ON tl_num = ti_se_num
        JOIN
    seat ON se_num = se_sc_num
        JOIN
    screen ON se_sc_num = sc_th_num
		join
	theater on th_num = sc_th_num;
...
FROM
    ...
    ticketing_list ON tl_ti_num = ti_num
        JOIN
    seat ON se_num = ti_se_num
WHERE
    sh_num = 1;
    
# 1번 스케줄에서 좌석들 예약 확인 하는 쿼리 (예약되면 tl_num를 보여주고, 아니면 null)
SELECT 
    se_name as 좌석, ifnull(tl_num, '0')
FROM
    seat
        JOIN
    screen ON sc_num = se_sc_num
    ...
    WHERE
        ti_sh_num = 1) as t ON ti_se_num = se_num
WHERE
    sh_num = 1;
...
# 1번 상영시간(웡카, CGV강남 1상영관 10:30 스케줄)을 예매된 수량을 조회
SELECT 
    COUNT(*)
FROM
    ticketing_list
        JOIN
    ticketing ON tl_ti_num = ti_sh_num
WHERE
    ti_sh_num = 1;
    
# 1번 상영시간(웡카, CGV강남 1상영관 10:30 스케줄)을 예매된 수량을 조회
select sum(ti_adult) + sum(ti_trrnager) as 예매수량 from ticketing where ti_sh_num = 1;
