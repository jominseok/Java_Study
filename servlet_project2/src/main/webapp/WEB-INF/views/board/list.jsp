...
<body>
	...
	<div class="container">
			...
			<div class="input-group mb-3">
				<select class="form-control" name="type">
					...
					<option value="bo_title"
						<c:if test='${pm.cri.type == "bo_title"}'>selected</c:if>>제목</option>
					<option value="bo_me_id"
						<c:if test='${pm.cri.type == "bo_me_id"}'>selected</c:if>>작성자</option>
				</select> <input type="text" class="" name="search" placeholder="검색어"
					value="${pm.cri.search}">
			...
			<tbody>
				<c:forEach items="${boardList}" var="board">
					<tr>
						...
						<td><a href="<c:url value = "/detail?num=${board.bo_num}" />">
								${board.bo_title}</a></td>
		...
		<ul class="pagination  justify-content-center">
			<c:if test="${pm.prev}">
				<li class="page-item"><c:url var="url" value="/board">
					...
					</c:url> <a class="page-link" href="${url}">이전으로</a></li>
			</c:if>
			<c:forEach begin="${pm.startPage}" end="${pm.endPage}" var="i">
				<li class="page-item <c:if test="${pm.cri.page == i}">active</c:if>"
					value="page"><c:url var="url" value="/board">
			...
			<c:if test="${pm.next}">
				<li class="page-item"><c:url var="url" value="/board">
						...
						<c:param name="search" value="${pm.cri.search}"></c:param>
						<c:param name="type" value="${pm.cri.type}"></c:param>
					</c:url> <a class="page-link" href="${url}">다음으로</a></li>
			</c:if>
		</ul>
