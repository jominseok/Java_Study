				...
				<label for="community" class="form-label">게시판:</label>
				 <select class="form-control" name="community" id="community">
					<c:forEach items="${list}" var="community">
						<option value="${community.co_num}"
							<c:if test="${board.bo_co_num == community.co_num}">selected</c:if>>${community.co_name}</option>
					</c:forEach>
				</select>
			</div>
			<div class="mb-3 mt-3">
				...
				<c:if test="${fileList!=null || fileList.size() != 0}">
					...
					<c:forEach items="${fileList}" var="file">
						<span class="form-control">${file.fi_ori_name} <a
							class="btn-del" href="#" data-target="${file.fi_num}">&times;</a>
						</span>
					</c:forEach>
