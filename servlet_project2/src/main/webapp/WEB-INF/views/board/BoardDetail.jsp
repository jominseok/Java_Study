			...
			<c:when test="${board != null}">
				...
				<div class="mb-3 mt-3">
					<label for="view" class="form-label">조회수:</label> <input
						type="text" class="form-control" readonly value="${board.bo_view}">
				</div>
				<div class="mb-3 mt-3">
				...
				<div class="mb-3 mt-3 clearfix">
					<button type="button" data-state="1"
						class="btn btn-outline-success btn-up float-start col-5">추천</button>
					<button type="button" data-state="-1"
						class="btn btn-outline-success btn-down float-end col-5">비추천</button>
				</div>
			</c:when>
			<c:otherwise>
...
//댓글들을 불러와서 화면에 출력하는 함수 : 현재 댓글 페이지 정보
...
//페이지네이션이 주어지면 댓글 페이지 네이션을 화면에 출력하는 함수
function displayCommentPagination(pm){
	console.log("여기도 들어ㅗ옴")
	let str = '';
	if(pm.prev){
		str += 
		`
			<li class="page-link"><a class="page-link" href="javascript:void(0);" data-page="\${pm.startPage-1}">이전</a></li>
		`
	}
	...
	if(pm.next){
		str += 
			`
				<li class="page-link"><a class="page-link" href="javascript:void(0);" data-page="\${pm.endPage+1}">다음</a></li>
			`
	}
