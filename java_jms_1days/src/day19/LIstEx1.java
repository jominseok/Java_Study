...
public class LIstEx1 {
	...
	public static void main(String[] args) {
		...
		/* - list3는 list인터페이스를 구현한 구현 클래스의 객체
		 * 중 아무나 올 수 있다.
		 * - list3 = new ArrayList<String>();//O
		 * - list3 = new LinkedList<String>();//O
		 * list4 = ArrayList클래스를 이용한 객체만 올 수 있다.
		 ...
		 * - 해당 클래스에 equals가 어떻게 오버라이딩 되어 있느냐에 따라 결과가 달라짐 
		 */
		System.out.println(list5.contains("abc"));
		
		/* indexOf(객체) : List, 리스트의 객체가 몇번지에 있느지 알려줌, 없으면 -1
		 * - 동작 방식은 contains와 같음 
		 */
		 ...
		 * - 정렬 기준은 정수로 알려주고, 0이면 같은 위치, 양수이면 뒤
		 * - CompareTo 인터페이스를 구현한 클래스의 객체가 필요
		 * - 일반적으로는 람다식을 이용하여 익명 객체를 생성해서 활용
		 */
		list5.sort((s1, s2)->s1.compareTo(s2));
		
		/* Collections.sort() : 리스트 타입이 comparable 인터페이스를 구현한 경우 
		 * 해당 기준을 이용하여 정렬
		 */
